...
### Phase 6 — Tauri, real IPC, the second window · **IN PROGRESS (Rust side done)**
...
1. Front end: install `@tauri-apps/api`; `ipc/tauri-client.ts` (a `Transport` over
   ...
   the registry (§2 rule 3). `registry.ts`'s `RustOwner` gains `sv2-studio` for
   `workspace` and `file_text`, the two it now answers.
2. Run the studio against `bun run dev` on a real workspace and look.
3. The pop-out editor window (IX-07) — last.
